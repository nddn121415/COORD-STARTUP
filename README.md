...
[**COORD 0.8 Mac prerelease**](https://github.com/nddn121415/COORD-STARTUP/releases/tag/desktop-v0.8.0) targets Apple silicon/macOS 13+. The tag-triggered desktop workflow builds and publishes its installer; availability depends on that workflow succeeding. It is ad-hoc signed, not Apple-notarized. It defaults to `https://coord-startup-control-plane.vercel.app`; `COORD_WEBSITE_URL` overrides this when building for another deployment. Use 0.8 or later for HTTPS projects; old clients get an update message.
