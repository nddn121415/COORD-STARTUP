...
**Public beta still needs hosting and sign-in setup.** Vercel serves the website and account API. Supabase Auth handles sign-in, and Supabase PostgreSQL stores account membership, invitations and desktop sessions. A separate always-on Node24/Linux hub stores shared source and device identities on its persistent private disk. The original contributor's computer can go offline while others continue. Git is not the live sharing transport. The hub operator can read shared source.
...
[**COORD 0.7 Mac prerelease**](https://github.com/nddn121415/COORD-STARTUP/releases/tag/desktop-v0.7.0) targets Apple silicon/macOS13+. The tag-triggered desktop workflow builds and publishes its installer; availability depends on that workflow succeeding. It is ad-hoc signed, not Apple-notarized. The published 0.7.0 app requires the website address on first sign-in. Builds from 0.7.1 default to `https://coord-startup-control-plane.vercel.app`; `COORD_WEBSITE_URL` still overrides it when building for another deployment.
