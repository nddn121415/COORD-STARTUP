...
Automatic `--wifi` mode explicitly contacts the public HyperDHT discovery network and opens UDP peer connectivity. Public IPs and temporary public keys are visible to that infrastructure. TLS capabilities and file bytes are not published to DHT records. The encrypted stream bridges only to the fixed loopback snapshot server, with connection limits and timeouts; it cannot name other local destinations. The receiver pins both the DHT identity and the TLS certificate. No hosted data-relay fallback is configured. Native dependencies are included from the lockfile in downloads and retain their package licenses.

## Desktop and collaboration portal

The website uses Sites' verified ChatGPT sign-in headers for browser identity and checks project membership server-side. Device pairing requires an explicit signed-in browser approval of a short-lived code; native API calls use a separate random bearer secret whose server record stores a hash. The desktop protects that secret and its X25519 private key using OS-backed Electron safeStorage. The renderer receives neither secret. Project-scoped MCP adapters communicate through an authenticated owner-only Unix socket while the desktop is running, exposing five metadata operations rather than the original twelve-tool PostgreSQL service.

The desktop creates a recipient-encrypted invitation and client-side AES-GCM encrypted snapshot for temporary HTTPS delivery. It prefers a direct connection and falls back to that ciphertext copy when needed. The hosted device public-key directory is trusted: this prototype does not protect against a compromised directory substituting recipient keys. The service never receives plaintext snapshot contents or the corresponding decryption keys from an honest client. Every relay read/write checks device validity and current project membership. Acknowledgment deletes the ciphertext; expired objects are inaccessible immediately and removed opportunistically on subsequent authenticated polling, not by an exact-time erasure guarantee.

Mac downloads include their runtime. Current early-access artifacts are ad-hoc signed, not Developer ID signed or notarized. A publicly trusted installation and automatic updates require production signing/release setup. The website, native shell, cryptographic delivery and five-tool agent adapter are tested; physical cross-network paired-user onboarding remains a manual validation boundary.
