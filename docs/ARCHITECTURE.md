...
This changes the original metadata-only prototype by adding an explicit opt-in file channel. Normal Git observation still never reads or uploads source content. The direct listener is loopback-only unless the sender explicitly chooses a reachable interface; it authenticates every transfer request. Opt-in `--wifi` mode adds HyperDHT public-key discovery and UDP hole punching, carrying pinned HTTPS inside an encrypted Noise stream. Discovery uses public infrastructure and shares network metadata only. A hosted relay fallback and continuous edit synchronization remain future work.
