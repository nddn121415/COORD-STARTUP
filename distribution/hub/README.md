...
Caddy obtains and renews TLS certificates. Keep port 4200 bound to loopback. Set `COORD_HUB_URL=https://your-hostname` for administration. The API requires bearer authentication for all `/v1` routes and does not accept browser-origin administration. The legacy `/v1` operator workflow does not require website accounts. The account-enabled website uses a separate server-side `COORD_PORTAL_TOKEN` and `/cloud/connect` when using Supabase; configure the same portal token on the hub and Vercel, and expose the hub through HTTPS. Never give the website the administration token.

## Website accounts and the tester experience

For the Supabase website flow, set `SUPABASE_URL` and `SUPABASE_SECRET_KEY` on the hub to the same project used by Vercel. Set `COORD_PORTAL_TOKEN` to a separate random 64-hex-character value in the private hub environment file. Set that same portal secret and `COORD_HUB_URL=https://your-hub-hostname` in Vercel, then redeploy the website. Compose passes these settings into the hub container. Supabase stores memberships and checks each cloud peer operation; the hub stores source, reservations, and transport identities. In cloud mode the legacy `/account` API is disabled. Leaving both Supabase values empty preserves the older standalone hub mode.
